...
export class FinancierosBancariaComponent implements OnInit {
  ...
  openEditModal(registro: any): void {
    ...
    this.scanError = null;
    this.isScanSuccess = registro.status === 'PAGADO' && !!registro.verification;
    ...
    // Asegurar que los controles permanezcan deshabilitados en cada apertura del modal
    this.editForm.get('estado')?.disable({ emitEvent: false });
    this.editForm.get('verificado')?.disable({ emitEvent: false });
    // Si el registro ya está pagado y verificado, permitimos que el admin
    // pueda gestionar manualmente estos campos.
  ...
  onSubmitEdit(): void {
    ...
    const isMarkingPaid = requestedEstado === 'PAGADO' && originalEstado !== 'PAGADO';
    const isMarkingVerified = requestedVerificado && !originalVerificacion;

    if ((isMarkingPaid || isMarkingVerified) && !this.isScanSuccess) {
      this.notificationService.error(
        'Debes subir y escanear un PDF válido antes de marcar el pago como PAGADO y verificado.'
      );
      return;
    }

    let nuevoEstado = originalEstado;
    let nuevaVerificacion = originalVerificacion;

    // Solo permitimos pasar de A PAGAR a PAGADO y verificado cuando el escaneo fue exitoso
    if (this.isScanSuccess && originalEstado !== 'PAGADO') {
      nuevoEstado = 'PAGADO';
    ...
    const verificadoPorUsuarioId =
      nuevaVerificacion && currentUser ? Number(currentUser.id) : undefined;
  ...
  get canSubmitEdit(): boolean {
    ...
    const isMarkingPaid = requestedEstado === 'PAGADO' && originalEstado !== 'PAGADO';
    const isMarkingVerified = requestedVerificacion && !originalVerificacion;
