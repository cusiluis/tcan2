...
export class ConfiguracionUsuariosComponent implements OnInit, OnDestroy {
  ...
  onCambiarContrasena(usuario: UsuarioView): void {
    // Acción preparada para flujo de cambio de contraseña por administrador
    // (pendiente de definir backend específico). Por ahora solo se notifica al usuario.
    alert(`Cambiar contraseña para el usuario: ${usuario.nombre_usuario}`);
  }
}
